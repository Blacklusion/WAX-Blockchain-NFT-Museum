...
namespace WebP.Experiment.Animation
{
    ...
    public static class WebPDecodeJob
    {
        ...
        public static UniTask<List<(byte[], int)>> StartJob(IntPtr decoder, int count)
        {
            ...
            if (managedBytes.Count <= 0 || managedBytes.Values.Count <= 0) return UniTask.FromResult(new List<(byte[], int)>());

            var sorted = managedBytes.OrderBy(kvp => kvp.Key).ToList().ConvertAll(kvp => kvp.Value);
            return UniTask.FromResult(sorted);
        }
    }
